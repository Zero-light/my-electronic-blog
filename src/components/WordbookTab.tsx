  ...
  return (
    <div className="flex flex-col w-full h-full pt-4 px-4 overflow-y-auto">
      ...
      <div className="flex justify-center gap-3 mb-4">
        ...
        <div className="glass-chip text-blue-300"><GraduationCap size={12} /> {stats.mastered}掌握</div>
        <div className="glass-chip text-yellow-300"><Brain size={12} /> {stats.due}待复习</div>
      </div>

      {/* Word list */}
      <div className="space-y-1.5 max-w-lg mx-auto w-full">
        {wordbook.map((entry, i) => {
          const statusColors: Record<string, string> = {
            new: 'text-blue-300/60', learning: 'text-yellow-300/60', mastered: 'text-green-300/60',
          };
          const statusLabels: Record<string, string> = {
            new: '新', learning: '学', mastered: '✓',
          };
          return (
            <div key={entry.wordId} className="flex items-center gap-3 px-4 py-2.5 glass-panel text-sm">
              ...
              <span className={`text-[11px] ${statusColors[entry.status]}`}>
